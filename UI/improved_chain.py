...
class PrepareImprovedAgent:
    ...
    def map_to_database(self, values)->str:
        ...
        match_query = """MATCH (p:Person|Movie)
                       WITH p, apoc.text.levenshteinSimilarity(coalesce(p.name, p.title), $value) AS similarity
                       WHERE similarity > 0.7  // Allow for "fuzzy" matching with a threshold of 0.7
                       RETURN coalesce(p.name, p.title) AS result, labels(p)[0] AS type, similarity
